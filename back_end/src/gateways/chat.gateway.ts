...
export class ChatGateway {
  ...
  broadcastEventToServer(event: string, message: string){
    return this.chatGatewayService.broadcastEventToServer(this.server, event, message);
  }

  broadcastEventToRoom(room: Room, event: string, message: string){
    return this.chatGatewayService.broadcastEventToRoom(this.server, room, event, message);
  }
