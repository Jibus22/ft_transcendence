...
export default function FormUser({ isPop, userName, fetchData}: Props) {
    ...
    const validationSchema = yup.object({
      ...
      const formik = useFormik({ initialValues: {
        ...
        onSubmit: async (values) => {
            ...
            await axios.patch('http://localhost:3000/me',  user, {withCredentials: true})
    ...
    return (
        <div className='w-100 h-100  d-flex flex-column' >
            ...
            <TextField sx={{width: 2/2}} name='nickname' id="outlined-disabled" label={placeHolder}  autoComplete='off' size={'small'} rows={1.2} 
                        placeholder={userName}
                        
                        value={formik.values.nickname} 
                        onChange={formik.handleChange}
                        error={formik.touched.nickname && Boolean(formik.errors.nickname)}
                        helperText={formik && formik.errors.nickname} disabled={disableInputPop()}
                        inputProps={{
                            maxLength: 18,
                        }}
