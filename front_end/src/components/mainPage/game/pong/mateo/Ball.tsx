...
export class Ball {
	...
	_intersect_segment(xa: number, ya: number, xb: number, yb: number, xc: number, yc: number, xd: number, yd: number): boolean {
	...
	_update(playerOne: Player, playerTwo: Player): number {
		...
		if (this.y + this.y_dir - this.size < 0 || this.y + this.y_dir + this.size > this.windowHeigth) this.y_dir *= -1;

		//collision avec le player two
		if (
			this._intersect_segment(
				this.x - this.x_dir,
				this.y - this.y_dir,
				this.x + this.size + this.x_dir,
				this.y + this.size + this.y_dir,
				playerTwo.x,
				playerTwo.y,
				playerTwo.x,
				playerTwo.y + playerTwo.size,
			)
		)
			this.x_dir *= -1;
		else if (
		...
		if (this.x > this.windowWidth) return 1;
